...
impl<C> Api<C> for Server<C> where C: Has<XSpanIdString> + Send + Sync
{
        ...
        context: &C) -> Result<GetMyselfResponse, ApiError>
    {
        ...
        info!("get_myself() - X-Span-ID: {:?}", context.get().0.clone());
    ...
    {
        let context = context.clone();
        info!("get_new_user() - X-Span-ID: {:?}", context.get().0.clone());
        Err("Generic failure".into())
    }
    ...
    {
        let context = context.clone();
        info!("post_query({:?}) - X-Span-ID: {:?}", request_body, context.get().0.clone());
        Err("Generic failure".into())
    }
